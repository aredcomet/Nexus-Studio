...
from model import T5Gemma2ForConditionalGeneration
...
def main():
    parser = argparse.ArgumentParser(description="Quantize T5Gemma-2 model weights to 4-bit or 8-bit in MLX")
    parser.add_argument("--config", type=str, default="models/t5gemma-2-270m-270m/config.json", help="Path to config.json")
    parser.add_argument("--weights", type=str, default="weights/t5gemma-2-270m-270m/weights.safetensors", help="Path to baseline float16/bfloat16 weights.safetensors")
    parser.add_argument("--output", type=str, default="weights/t5gemma-2-270m-270m-4bit/weights.safetensors", help="Path to save quantized weights")
    ...
    if args.mode == "mxfp4":
        args.group_size = 32
        args.bits = 4
        if args.output == "weights/t5gemma-2-270m-270m-4bit/weights.safetensors":
            args.output = "weights/t5gemma-2-270m-270m-mxfp4/weights.safetensors"
    elif args.mode == "mxfp8":
        args.group_size = 32
        args.bits = 8
        if args.output == "weights/t5gemma-2-270m-270m-4bit/weights.safetensors":
            args.output = "weights/t5gemma-2-270m-270m-mxfp8/weights.safetensors"
    elif args.mode == "nvfp4":
        args.group_size = 16
        args.bits = 4
        if args.output == "weights/t5gemma-2-270m-270m-4bit/weights.safetensors":
            args.output = "weights/t5gemma-2-270m-270m-nvfp4/weights.safetensors"
    ...
    model = T5Gemma2ForConditionalGeneration(config_dict)
