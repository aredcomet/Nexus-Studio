...
* **[model.py](file:///Users/bran/src/play/llm/recurrentgemma/model.py)**: Native MLX implementation of the Griffin architecture, including:
...
* **[convert.py](file:///Users/bran/src/play/llm/recurrentgemma/convert.py)**: Weight converter that maps Hugging Face PyTorch safetensors into MLX format, automatically transposing Conv1D weights and explicitly tying word embedding weights.
* **[generate.py](file:///Users/bran/src/play/llm/recurrentgemma/generate.py)**: Autoregressive causal text generation runner, using the tokenizer's chat template and custom caching to run step-by-step decoding.
...
```bash
.venv/bin/python recurrentgemma/convert.py \
...
```bash
PYTHONPATH=recurrentgemma .venv/bin/python recurrentgemma/generate.py \
...
```bash
.venv/bin/python recurrentgemma/quantize.py \
  --config models/recurrentgemma-2b-it/config.json \
...
You can start a local FastAPI server conforming to the OpenAI Chat Completions API standard:
```bash
.venv/bin/python recurrentgemma/server.py \
  --config models/recurrentgemma-2b-it/config.json \
  --weights weights/recurrentgemma-2b-it-mxfp4/weights.safetensors \
  --tokenizer models/recurrentgemma-2b-it \
  --port 8000
```
This server supports `/v1/models` and `/v1/chat/completions` (both streaming and non-streaming modes) and integrates directly with any standard OpenAI clients (e.g., custom chat frontends, LiberaChat, LangChain, etc.).
